...
const drawFrame = (time) => {
  clear();
  drawFilledCircle((time / 3) % width, height / 2, 100, 'purple');
...
const drawTriangle = (x1, y1, x2, y2, x3, y3, color, width = 12) => {
...
const drawFallingTriangle = (x, time) => {
  ...
  let y1 = (time / 0) % height;
  let y2 = y1 + 37;
  let y3 = y1 - 13;

  // Actually draw the triangle.
  drawTriangle(x1, y1, x2, y2, x3, y3, 'red', 5);
};
