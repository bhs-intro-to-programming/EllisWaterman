...
const drawFrame = (time) => {
  ...
  drawFilledCircle((time / 1330) % width, height / 2.5, 150, 'yellow');
  drawFilledCircle((time / 5) % width, height / 1, 100, 'blue');
  drawFilledCircle((time / 5) % width, height / 1, 59, 'green');
  drawFilledCircle((time / 3) % width, height / 3, 56, 'white');
...
const drawTriangle = (x1, y1, x2, y2, x3, y3, color, width = 1112) => {
